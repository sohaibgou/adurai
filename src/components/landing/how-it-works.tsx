...
const STEPS = [
  {
    ...
    mockup: (
      <div style={{ padding: "16px", background: "#FAF8F5", borderRadius: 12, border: "1px solid rgba(255,60,172,0.12)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, height: 2, background: "linear-gradient(90deg, #FF3CAC, #FF6B35)" }} />
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #FF3CAC, #FF6B35)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ padding: "16px", background: "#FAF8F5", borderRadius: 12, border: "1px solid rgba(255,60,172,0.12)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6, marginBottom: 10 }}>
          {[1,2,3,4,5,6,7,8,9].map(i => (
            <div key={i} style={{ height: 28, borderRadius: 6, background: i === 5 ? "linear-gradient(135deg, #FF3CAC, #FF6B35)" : "#fff", border: `1px solid ${i === 5 ? "transparent" : "#e5e7eb"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {i === 5 && <div style={{ width: 10, height: 10, borderRadius: "50%", background: "rgba(255,255,255,0.8)" }} />}
            </div>
          ))}
        </div>
        <div style={{ height: 2, borderRadius: 2, background: "linear-gradient(90deg, #FF3CAC, #FF6B35)" }} />
      </div>
    ),
  },
  {
      ...
      <div style={{ padding: "16px", background: "#f0fdf9", borderRadius: 12, border: "1px solid rgba(0,184,148,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1, height: 4, borderRadius: 2, background: "#e5e7eb" }}>
            <div style={{ width: "65%", height: "100%", borderRadius: 2, background: "linear-gradient(90deg, #FF3CAC, #FF6B35)" }} />
          </div>
        ...
        <FadeIn>
          <div className="text-center mb-20">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6" style={{ background: "rgba(255,60,172,0.08)", border: "1px solid rgba(255,60,172,0.18)" }}>
              <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#FF3CAC" }}>Engine Workflow</span>
            </div>
            <h2 className="font-heading" style={{ fontSize: "clamp(36px, 5vw, 64px)", fontWeight: 900, letterSpacing: "-0.03em", color: "#0d0d1a", lineHeight: 1.05 }}>
              Efficiency in{" "}
              <span style={{ fontStyle: "italic", background: "linear-gradient(135deg, #FF3CAC, #FF6B35)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>3 Steps.</span>
            </h2>
          ...
          {STEPS.map((step, i) => (
            <FadeIn key={step.number} delay={i * 0.1}>
              <motion.div
                ...
                style={{ background: "#ffffff", border: "1px solid #E8E5E0", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}
              >
