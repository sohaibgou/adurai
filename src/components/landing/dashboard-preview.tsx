...
const BATTLE_PLAN = [
  { day: "Day 1", action: "Scale your best ROAS campaign by 20%", badge: "Quick Win", badgeColor: "#00b894", badgeBg: "rgba(0,184,148,0.12)" },
  { day: "Day 2", action: "Launch 3 new creatives on your top performer", badge: "Strategic", badgeColor: "#6c5ce7", badgeBg: "rgba(108,92,231,0.12)" },
  { day: "Day 3", action: "Expand to Lookalike 2–5% audience", badge: "Strategic", badgeColor: "#6c5ce7", badgeBg: "rgba(108,92,231,0.12)" },
];
...
const INSIGHTS = [
  ...
  { text: "Zero retargeting layer detected — add warm audience campaign to increase ROAS by estimated 0.8x", color: "#e040fb", bg: "rgba(224,64,251,0.08)", border: "rgba(224,64,251,0.2)" },
];

function BrowserShell({ children, url = "app.adur.ai/dashboard" }: { children: React.ReactNode; url?: string }) {
  return (
    <div style={{ background: "#ffffff", borderRadius: 18, overflow: "hidden" }}>
      <div style={{ background: "#f8f8fc", borderBottom: "1px solid #f0f0f5", padding: "10px 16px", display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ display: "flex", gap: 6 }}>
          {["#ff5f57", "#febc2e", "#28c840"].map(c => (
            <div key={c} style={{ width: 10, height: 10, borderRadius: "50%", background: c }} />
          ))}
        </div>
        <div style={{ flex: 1, background: "#fff", borderRadius: 6, padding: "4px 12px", border: "1px solid #f0f0f5", fontSize: 11, color: "#9ca3af", textAlign: "center" }}>
          {url}
        </div>
      </div>
      {children}
    ...
    <div style={{ padding: "20px 24px", background: "#ffffff" }}>
      ...
      <div style={{ display: "flex", gap: 20, borderBottom: "1px solid #f0f0f5", marginBottom: 14 }}>
        {["Campaigns", "Ad Sets", "Ads"].map((t, i) => (
          <span key={t} style={{ fontSize: 13, fontWeight: i === 0 ? 600 : 400, color: i === 0 ? "#6c5ce7" : "#9ca3af", paddingBottom: 10, borderBottom: i === 0 ? "2px solid #6c5ce7" : "2px solid transparent", fontFamily: "var(--font-inter)", cursor: "default" }}>{t}</span>
        ))}
      </div>
      ...
      {CAMPAIGNS.map((c, i) => (
        <div key={c.name} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 80px 90px 90px 70px", gap: 8, padding: "10px 0", borderBottom: "1px solid #f9f9f9", alignItems: "center" }}>
          <span style={{ fontSize: 12, fontWeight: 500, color: "#6c5ce7", fontFamily: "var(--font-inter)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</span>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: c.roas_color, fontFamily: "var(--font-inter)" }}>{c.roas}x</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ padding: "24px", background: "#0d0d1a", minHeight: 320 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, #6c5ce7, #e040fb)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ marginLeft: "auto", fontSize: 11, fontWeight: 600, color: "#00b894", background: "rgba(0,184,148,0.15)", border: "1px solid rgba(0,184,148,0.25)", padding: "4px 10px", borderRadius: 100, fontFamily: "var(--font-inter)" }}>
          3 actions ready
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {BATTLE_PLAN.map((item, i) => (
          ...
          >
            ...
            {/* Badge */}
            <span style={{ fontSize: 11, fontWeight: 600, color: item.badgeColor, background: item.badgeBg, border: `1px solid ${item.badgeColor}30`, padding: "4px 10px", borderRadius: 100, flexShrink: 0, fontFamily: "var(--font-inter)" }}>
              {item.badge}
            </span>
          </div>
        ))}
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 8 }}>
        <CheckCircle2 style={{ width: 14, height: 14, color: "#00b894" }} />
...
const MOCK_CONCEPTS = [
  {
    ...
    headerBg: "#0a0a0f",
    directionColor: "#e040fb",
    // mock ad layers
    adBg: "linear-gradient(145deg, #0d0020 0%, #0a0a0f 55%, #130025 100%)",
    glowColor: "rgba(108,92,231,0.5)",
    productBg: "linear-gradient(135deg, #6c5ce7 0%, #e040fb 100%)",
    productShape: "tall",
    adTextColor: "#ffffff",
    ctaBg: "linear-gradient(135deg, #6c5ce7, #e040fb)",
    ctaColor: "#ffffff",
  },
  {
    ...
    headerBg: "#f5f3ff",
    directionColor: "#6c5ce7",
    adBg: "linear-gradient(150deg, #ffffff 0%, #f0f4ff 60%, #ede9ff 100%)",
    glowColor: "rgba(108,92,231,0.18)",
    productBg: "linear-gradient(135deg, #a8c8f8 0%, #7ec8e3 100%)",
    productShape: "round",
    adTextColor: "#0d0d1a",
    ctaBg: "#0d0d1a",
    ctaColor: "#ffffff",
  },
  {
    ...
    headerBg: "#fff3e0",
    directionColor: "#e17055",
    adBg: "linear-gradient(135deg, #ff6348 0%, #ff9f43 55%, #ffeaa7 100%)",
  ...
  return (
    <div style={{ position: "relative", aspectRatio: "1", background: concept.adBg, overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "10px" }}>
      ...
      <div style={{
        position: "absolute",
        left: "50%", top: "46%",
        transform: `translate(-50%, -50%)${isDiamond ? " rotate(12deg)" : ""}`,
        width: isTall ? "28%" : isRound ? "38%" : "32%",
        height: isTall ? "52%" : isRound ? "38%" : "32%",
        borderRadius: isTall ? "100px" : isRound ? "50%" : "12px",
        background: concept.productBg,
    ...
    <div style={{ padding: "18px 20px", background: "#ffffff" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: "linear-gradient(135deg, #6c5ce7, #e040fb)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          <div style={{ marginLeft: "auto", fontSize: 10, fontWeight: 600, color: "#6c5ce7", background: "rgba(108,92,231,0.08)", border: "1px solid rgba(108,92,231,0.18)", padding: "3px 9px", borderRadius: 100, fontFamily: "var(--font-inter)", flexShrink: 0, whiteSpace: "nowrap" }}>
        ...
        {MOCK_CONCEPTS.map((concept, i) => (
          <div key={i} style={{ borderRadius: 10, overflow: "hidden", border: "1px solid #f0f0f5", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ display: "flex", gap: 5, padding: "7px 8px", background: "#fafafa", borderTop: "1px solid #f0f0f5" }}>
              <div style={{ flex: 1, textAlign: "center", padding: "4px 0", borderRadius: 5, background: "rgba(108,92,231,0.08)", fontSize: 8.5, fontWeight: 600, color: "#6c5ce7", fontFamily: "var(--font-inter)", cursor: "default" }}>↓ Download</div>
      ...
      {/* Generate CTA */}
      <div style={{ width: "100%", padding: "9px", borderRadius: 8, background: "linear-gradient(135deg, #6c5ce7, #e040fb)", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, boxShadow: "0 4px 16px rgba(108,92,231,0.3)" }}>
...
function InsightsContent() {
  return (
    <div style={{ padding: "24px", background: "#ffffff", minHeight: 320 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, #fdcb6e, #e040fb)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Lightbulb style={{ width: 16, height: 16, color: "#fff" }} />
        </div>
        <div>
          <p style={{ fontSize: 15, fontWeight: 700, color: "#0d0d1a", fontFamily: "var(--font-inter)" }}>AI Insights</p>
          <p style={{ fontSize: 12, color: "#9ca3af", fontFamily: "var(--font-inter)" }}>Patterns and anomalies detected in your account</p>
        </div>
        <div style={{ marginLeft: "auto", fontSize: 11, fontWeight: 600, color: "#6c5ce7", background: "rgba(108,92,231,0.1)", border: "1px solid rgba(108,92,231,0.2)", padding: "4px 10px", borderRadius: 100, fontFamily: "var(--font-inter)" }}>
          3 insights
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {INSIGHTS.map((insight, i) => (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.55, fontFamily: "var(--font-inter)" }}>{insight.text}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function DashboardPreview() {
  ...
  return (
    <section className="relative py-28 px-6 overflow-hidden" style={{ background: "#0d0d1a" }}>
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div style={{ position: "absolute", top: "30%", left: "10%", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(108,92,231,0.15) 0%, transparent 70%)", filter: "blur(80px)" }} />
        <div style={{ position: "absolute", top: "40%", right: "5%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(224,64,251,0.10) 0%, transparent 70%)", filter: "blur(80px)" }} />
      </div>

      <div className="relative z-10 max-w-6xl mx-auto">
        <FadeIn>
          <div className="text-center mb-12">
            <h2 className="font-heading" style={{ fontSize: "clamp(36px, 5vw, 60px)", fontWeight: 900, letterSpacing: "-0.03em", color: "#ffffff", lineHeight: 1.05 }}>
              Discover the{" "}
              <span style={{ background: "linear-gradient(135deg, #6c5ce7, #e040fb)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                Adur.ai
              </span>{" "}
              platform
            </h2>
            <p style={{ fontSize: 17, color: "rgba(255,255,255,0.5)", marginTop: 14, fontFamily: "var(--font-inter)" }}>
          ...
          <div className="flex items-center justify-center gap-2 mb-8 flex-wrap">
            {TABS.map((tab, i) => (
              <button
                ...
                style={{
                  background: activeTab === i ? "linear-gradient(135deg, #6c5ce7, #e040fb)" : "rgba(255,255,255,0.07)",
                  color: activeTab === i ? "#fff" : "rgba(255,255,255,0.5)",
                  border: activeTab === i ? "none" : "1px solid rgba(255,255,255,0.1)",
                  fontFamily: "var(--font-inter)",
                  boxShadow: activeTab === i ? "0 4px 20px rgba(108,92,231,0.35)" : "none",
                }}
              >
                {tab}
              </button>
        ...
        {/* Product mockup with gradient border */}
        <FadeIn delay={0.14}>
          <div
            ...
            style={{
              ...
              boxShadow: "0 0 80px rgba(108,92,231,0.35), 0 0 120px rgba(224,64,251,0.2)",
            }}
          >
            {/* Browser shell wrapping animated content */}
